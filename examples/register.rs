...
fn get(args: &Args) {
    ...
    let request = encode(&Query::Get, SizeLimit::Infinite).unwrap();
...
fn put(args: &Args) {
    ...
    let request = encode(&proposal, SizeLimit::Infinite).unwrap();
...
impl state_machine::StateMachine for RegisterStateMachine {
    ...
    fn apply(&mut self, proposal: &[u8]) -> Vec<u8> {
        ...
        let message = match decode::<Proposal>(&proposal) {
            Ok(proposal) => proposal,
            Err(err) => return format!("{}", err).into_bytes(),
        };

        // Encoding the current value should never fail.
        let response = encode(&Response::Ok(self.value.clone()),
    ...
    fn query(&self, query: &[u8]) -> Vec<u8> {
        if let Err(err) = decode::<Query>(&query) {
            return format!("{}", err).into_bytes();
        }

        // Encoding the current value should never fail.
        encode(&Response::Ok(self.value.clone()),
                                  SizeLimit::Infinite).unwrap()
    }
